...
- **Every login branch spends what a verification spends.** An unknown name, a user
  ...
  the network. Q1.401.
- **Login answers one thing.** Unknown name, wrong password and no-password-row are
  all `401 invalid_login`. `403 user_disabled` comes *only after* the password
  verified, which leaks nothing to somebody who does not already hold it.
